...
async function syncAptitudeWithServer() {
    try {
        // Важно: WebApp часто открывается с того же домена, что и API.
        // Тогда API_BASE пустой строкой, но обращаться к /api/* всё равно можно.
        // Раньше здесь был guard `if (!API_BASE) return;` — из‑за него
        // синхронизация не выполнялась и после админского «Сбросить всю статистику»
        // локальные очки/рекомендации/проф-тест возвращались при повторном входе.
