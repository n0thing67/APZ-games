...
// Переиспользуемый массив пустых ячеек
const emptyCells = [];
...
function init2048() {
    ...
    for (let r = 0; r < SIZE; r++) {
        board2048[r] = Array(SIZE).fill(null);
    }
    ...
    // Засекаем время старта 2048 (для очков уровня)
    levelStartTime = Date.now();
}
...
function check2048Status() {
    ...
    // Если пустых нет — проверяем возможность слияния
    if (!hasEmpty) {
        for (let r = 0; r < SIZE; r++) {
            for (let c = 0; c < SIZE; c++) {
                const tile = board2048[r][c];
                const val = tile.val;
                if (c < SIZE - 1 && board2048[r][c + 1] && board2048[r][c + 1].val === val) {
                    canMerge = true;
                    break;
                }
                if (r < SIZE - 1 && board2048[r + 1][c] && board2048[r + 1][c].val === val) {
                    canMerge = true;
                    break;
                }
            }
